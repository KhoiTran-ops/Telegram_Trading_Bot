...
def parse_chart_args(args: list[str]) -> ChartOptions:
    ...
    period = args[1].lower() if len(args) > 1 else "6m"
    if period not in PERIOD_SESSIONS:
        raise ValueError("Khoảng thời gian hỗ trợ: 3m, 6m, 1y")
    indicators = tuple(dict.fromkeys(
        item.strip().lower() for item in (args[2].split(",") if len(args) > 2 else ("ema", "rsi"))
        if item.strip()
    ))
    unknown = set(indicators) - set(SUPPORTED_INDICATORS)
    if unknown:
        raise ValueError("Chỉ báo hỗ trợ: ema, rsi, macd, obv")
    return ChartOptions(symbol, period, indicators)
...
def render_candlestick(bars: list[Bar], options: ChartOptions, output_dir: Path) -> Path:
    selected = bars[-PERIOD_SESSIONS[options.period]:]
    if len(selected) < 20:
    ...
    ratios = [4, 1] + [1.15] * len(panels)
    figure, axes = plt.subplots(
        2 + len(panels), 1, figsize=(12, 7 + len(panels) * 1.4),
        sharex=True, gridspec_kw={"height_ratios": ratios}, constrained_layout=True,
    )
    price_ax, volume_ax = axes[0], axes[1]
    colors = []
    ...
    if "ema" in options.indicators:
        price_ax.plot(_ema(closes, 20), color="#f39c12", linewidth=1.4, label="EMA20")
        price_ax.plot(_ema(closes, 50), color="#3498db", linewidth=1.4, label="EMA50")
    ...
    price_ax.set_title(f"{options.symbol} · {options.period.upper()} · Nến ngày", fontweight="bold")
    price_ax.set_ylabel("Giá")
    volume_ax.bar(range(len(selected)), [bar.volume for bar in selected], color=colors, width=.7, alpha=.75)
    ...
    if "macd" in panels:
        ...
        axes[panel_index].plot(macd, color="#2980b9", label="MACD")
        axes[panel_index].plot([None] * offset + signal, color="#e67e22", label="Signal")
    ...
    if "obv" in panels:
        obv = [0]
        for before, current in zip(selected, selected[1:]):
        ...
        axes[panel_index].plot(obv, color="#2c3e50", linewidth=1.1)
